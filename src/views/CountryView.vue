<template>
  <BaseLayout :title="title">
    <div class="container mx-auto px-3 lg:px-32 pb-8">
      <RouterLink to="/crisishotline" class="text-primary underline font-medium inline-block mt-10">
      ...
      <ul v-if="hasValidContent" class="list-disc text-lg space-y-4 ms-5 md:ms-0 external-link">
      ...
      <div class="[&>p]:mb-4">
        ...
        <p>
          LifeLines Canada is in the process of gathering and organizing publicly available crisis and suicide support resources for this country.
        </p>
        <p>
          We are committed to ensuring that all information shared on this platform is accurate and responsibly presented. This page will be updated as listings become available.
        </p>
        <p>
          If you require immediate assistance, please contact local emergency services in your area.
        </p>
        <p>
          If you represent a verified organization providing crisis or suicide support and would like your service included, please contact us at:
          <a href="mailto:connect@lifelinesproject.com" class="underline font-medium text-primary">connect@lifelinesproject.com</a>.
        </p>
...
const title = computed(() => `${countryData.value.countryName} - Crisis Hotlines`)

const hasValidContent = computed(() => {
  const content = countryData.value.content
  return content && content.length > 0 && !(content.length === 1 && content[0] === '')
})
